...
module.exports = function (options) {
    ...
    return function (files, metalsmith, done) {
                ...
                if (fs.lstatSync(item.path).isDirectory()) {
                    ...
                    item.children.forEach(function(item){
                        if (fs.lstatSync(item.path).isDirectory()) {
                            ...
                            // la stessa categoria ricorre su piu' giorni:
                            // l'array si crea SOLO la prima volta, altrimenti
                            // ogni giorno cancellava le foto dei giorni
                            // precedenti e la pagina categoria mostrava solo
                            // l'ultimo giorno
                            if (!gallery[categoria]) {
                                gallery[categoria] = [];
                            }
                            item.children.forEach(function (item) {
        ...
        var categorie = Array.from(categorieSet);

        // Ordine cronologico nelle pagine: i filename del diario sono
        // YYYY.MM.DD_HH.MM.SS.<ext>, quindi l'ordine lessicografico del
        // nome coincide con l'ordine di tempo (l'ordine di readdirSync non
        // e' garantito e le stesse categorie raccolgono foto da piu' giorni).
        Object.keys(gallery).forEach(function (chiave) {
            gallery[chiave].sort(function (a, b) {
                return a.filename < b.filename ? -1 : (a.filename > b.filename ? 1 : 0);
            });
        });

        categorie.forEach(item => console.log(item));
        ...
        // l'indice del diario elenca le categorie in ordine alfabetico
        // (l'ordine di scoperta dipende dalla scansione delle cartelle)
        categorie.sort();
