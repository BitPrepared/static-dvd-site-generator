...
module.exports = function (options) {
    ...
    return function (files, metalsmith, done) {
            ...
            filteredTree.children.forEach(function(item){
                if (fs.lstatSync(item.path).isDirectory()) {
                    ...
                    item.children.forEach(function(item_child){
                        if (fs.lstatSync(item_child.path).isDirectory()) {
                            debug("categoria: " + item_child.name);
                            const categoria = item_child.name;
                            categorie.push(categoria);
                            gallery[categoria] = [];
                            item_child.children.forEach(function (item_photo) {
                                debug("foto: " + item_photo.name);
                                const name = path.basename(item_photo.name, path.extname(item_photo.name));
                                var obj = {
                                    "name": name,
                                    "filename": path.basename(item_photo.path),
                                    "path": item_photo.path.replace(options.imagesDirectory,''),
                                    "pathThumb": item_photo.path.replace(options.imagesDirectory, '').replace(path.basename(item_photo.path),'') + name + ".jpg"
                                };
        ...
        const categorieFiltered = categorie.filter((item, index) => categorie.indexOf(item) === index);
        ...
        Object.keys(files).forEach(function (file) {
            ...
            files[file][options.categoriesKey] = categorieFiltered || [];
        });
