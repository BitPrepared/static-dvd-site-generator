...
CFG="$TMP/campo.json"
echo '{"inizio": "22/08/2023", "fine": "26/08/2023"}' > "$CFG"
run_ok "config valida" "file" "$NODE_BIN" "$SCRIPT" --config "$CFG"
CFG_VUOTO="$TMP/campo_vuoto.json"
echo '{}' > "$CFG_VUOTO"
run_ko "config senza campi inizio/fine" "inizio" "$NODE_BIN" "$SCRIPT" --config "$CFG_VUOTO"
CFG_CATTIVO="$TMP/campo_cattivo.json"
echo '{"inizio": "pippo", "fine": "26/08/2023"}' > "$CFG_CATTIVO"
run_ko "config con inizio invalida" "formato" "$NODE_BIN" "$SCRIPT" --config "$CFG_CATTIVO"
CFG_FINE_CATTIVA="$TMP/campo_fine_cattiva.json"
echo '{"inizio": "22/08/2023", "fine": "pippo"}' > "$CFG_FINE_CATTIVA"
run_ko "config con fine invalida" "formato" "$NODE_BIN" "$SCRIPT" --config "$CFG_FINE_CATTIVA"
CFG_MESI="$TMP/campo_mesi.json"
echo '{"inizio": "28/08/2023", "fine": "02/09/2023"}' > "$CFG_MESI"
run_ko "config a cavallo di mesi" "mesi" "$NODE_BIN" "$SCRIPT" --config "$CFG_MESI"
CFG_ORDINE="$TMP/campo_ordine.json"
echo '{"inizio": "26/08/2023", "fine": "22/08/2023"}' > "$CFG_ORDINE"
run_ko "config con fine prima di inizio" "precede" "$NODE_BIN" "$SCRIPT" --config "$CFG_ORDINE"
